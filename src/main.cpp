...
#if defined(NDEBUG)
# error "Sperry cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x966d0f8849e085dccb3204c64f6198fbdf8da883b5b959b351b3d294895c70c3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sperry: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sperry Signed Message:\n";
...
{
    // Sperry: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Sperry: 1 day
static const int64 nTargetSpacing = 1 * 60; // Sperry: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sperry: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sperry: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x757ce20d57cd73675be7b6ccd4ba80d575002733ce15a5728fca0a86eb55674f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Sperry Coin - Iluy Sperry";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1525217260;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086959057;

        if (fTestNet)
        {
            block.nTime    = 1525217140;
            block.nNonce   = 385292302;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x7b7b1b9c2b3c6dee588e3db8de9b4a35088c2ade708eb9f2ce5a31923db87954"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Sperry: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SperryMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SperryMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SperryMiner(CWallet *pwallet)
{
    printf("SperryMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sperry-miner");
    ...
    try { loop {
        ...
        printf("Running SperryMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SperryMiner, pwallet));
}
